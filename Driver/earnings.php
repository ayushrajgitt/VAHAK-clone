...
// Total Pending
$stmt = $conn->prepare("SELECT SUM(amount) as total FROM payments p JOIN shipments s ON p.shipment_id = s.id WHERE s.driver_id = ? AND p.status IN ('pending', 'paid')");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$res = $stmt->get_result()->fetch_assoc();
...
$payments = [];
$stmt = $conn->prepare("SELECT p.*, s.pickup_city, s.drop_city FROM payments p JOIN shipments s ON p.shipment_id = s.id WHERE s.driver_id = ? ORDER BY p.id DESC");
$stmt->bind_param("i", $user_id);
$stmt->execute();
...
<div class="dashboard">
    ...
    <div class="content">
        <h2>💰 My Earnings</h2>
        <p style="margin-bottom: 20px; color: gray;">Track your revenue and pending settlements.</p>
        
        <div class="cards">
            <div class="card">
                <h2>Total Settled Earnings</h2>
